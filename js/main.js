...
function showArtworks(arr) {
    ...
    .then(item => {
        console.log(item)
        document.querySelector('h2').innerText = item.data.title;
        document.querySelector('img').src = `https://www.artic.edu/iiif/2/${item.data.image_id}/full/400,/0/default.jpg`
        document.querySelector('img').alt = item.data.thumbnail.alt_text
